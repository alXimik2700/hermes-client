"""
Aigena Messenger v3 - Full Voice Pipeline
...
WHISPER_CLI = "D:/whisper-blas/Release/whisper-cli.exe"
WHISPER_MODEL = "D:/whisper.cpp/models/ggml-base.bin"
PIPER_VOICE = "ru_RU-denis-medium"
PIPER_MODEL_DIR = "D:/piper_voices"
...
def process_text(mid, text):
    ...
    try:
        reply = call_hermes(text) or "[Aigena is busy]"
        db_save("ai", reply)
...
def process_file(mid, filepath, stored_name):
    with _agent_state["lock"]:
        _agent_state["status"] = "thinking"
    try:
        reply = call_hermes(f"[File uploaded: {stored_name}]") or "[Aigena is busy]"
        db_save("ai", reply)
    finally:
        with _agent_state["lock"]: _agent_state["status"] = "online"
