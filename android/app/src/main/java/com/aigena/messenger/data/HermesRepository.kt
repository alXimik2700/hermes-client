package com.aigena.messenger.data
...
class HermesRepository(private val dao: MessageDao) {
    ...
    suspend fun fetchAgentStatus(): AgentStatus? = withContext(Dispatchers.IO) {
        try {
            val req = Request.Builder()
                .url("${com.aigena.messenger.AppConfig.currentServerUrl}api/agent/status")
                .header("Authorization", "Bearer ${com.aigena.messenger.AppConfig.API_TOKEN}")
                .get().build()
    ...
    suspend fun fetchAttachments(type: String, limit: Int = 30): List<AttachmentMsg> = withContext(Dispatchers.IO) {
        try {
            val req = Request.Builder()
                .url("${com.aigena.messenger.AppConfig.currentServerUrl}api/messages/attachments?type=$type&limit=$limit")
                .header("Authorization", "Bearer ${com.aigena.messenger.AppConfig.API_TOKEN}")
                .get().build()
            val resp = httpClient.newCall(req).execute()
