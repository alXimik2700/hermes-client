...
object AppConfig {
    ...
    fun initFromPrefs(ctx: android.content.Context) {
        val prefs = ctx.getSharedPreferences("hermes_config", android.content.Context.MODE_PRIVATE)
        currentServerUrl = prefs.getString("server_url", BASE_URL) ?: BASE_URL
    }

    fun isRemoteMode(ctx: android.content.Context): Boolean {
        return ctx.getSharedPreferences("hermes_config", android.content.Context.MODE_PRIVATE)
            .getBoolean("remote_mode", false)
    }

    fun setRemoteMode(ctx: android.content.Context, enabled: Boolean) {
        ctx.getSharedPreferences("hermes_config", android.content.Context.MODE_PRIVATE)
